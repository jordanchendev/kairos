    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="mt-2 text-2xl font-semibold text-[hsl(var(--foreground))]">{formatCompactCurrency(latestPoint.total_nav)}</div>
          <div className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">
            Latest NAV on {formatDateLabel(latestPoint.date)} across the {timeRangeLabel} window
          </div>
        </div>
        <div className="rounded-[22px] border border-[hsl(var(--border))] bg-[hsla(190,91%,37%,0.08)] px-4 py-3">
          <div className="text-[11px] uppercase tracking-[0.24em] text-[hsl(var(--muted-foreground))]">Latest NAV</div>
          <div className="mt-2 text-lg font-semibold text-[hsl(var(--foreground))]">{formatCompactCurrency(latestPoint.total_nav)}</div>
          <div className="mt-1 text-sm text-emerald-200">{formatPercent(change, true)} vs first point</div>
        </div>
      </div>

      <div className="rounded-[24px] border border-[hsl(var(--border))] bg-[linear-gradient(180deg,hsla(190,91%,37%,0.1),transparent)] p-4">
